...
function initializeFormLogin() {
    ...
    inputs.forEach(input => {
        ...
        // Evento de blur para validação individual do campos
        input.addEventListener('blur', function() {
            validateFieldLogin(this.name, this.value)
        })
    })
...
function validateFieldLogin(fieldName, value) {
    let isValid = true
    let errorMessage = ''
...
function validateFormLogin() {
    ...
    for(const fieldName in formDataLogin) {
        const isFieldValid = validateFieldLogin(fieldName, formDataLogin[fieldName])
        if(!isFieldValid) {
            isFormValid = false
        }
    }
...
function handleFormLogar() {
    ...
    for (let i = 0; i < formElements.length; i++) {
        const element = formElements[i]
        if (element.name && element.value !== undefined) {
            formDataLogin[element.name] = element.value
        }
    }
    ...
    //Mostrar loading
    logarBtn.disabled = true
    logarBtn.classList.add('loading')
    logarBtn.textContent = 'Logando...'
    ...
    setTimeout(() => {
        ...
        // Remover loading
        logarBtn.disabled = false
        logarBtn.classList.remove('loading')
        logarBtn.textContent = "Logar na conta"
    }, 2000)

    function resetFormLogin() {
        const form = document.getElementById('loginForm')
        form.reset()
        formDataLogin = {email: '', password: ''}
        for (const fieldName in formDataLogin) {
            clearFieldError(fieldName)
        }
        formErrors = {}
